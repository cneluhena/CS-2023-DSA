...
struct HashTable{
    ...
    int hashfunc(string user_name){
        ...
        int hash = 0;
	    //add your code below
        int string_length = user_name.length();
        for (int i=0; i < string_length; i++){
            sum += int(user_name[i]);  //suming the ascii values of each character in the string
        };
    ...
    void insert(string user_name,string user_password){
        ...
        if (empty){
            ...
            cout << "Slot has already occupied.";
        }
    ...
    void hash_lookup(string user_name){
        ...
        if (empty){
            cout << "There is no password for this username." ;
        }
        else{
